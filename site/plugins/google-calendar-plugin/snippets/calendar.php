...
if ($attributes == null) {
    foreach ($obj['items'] as $event) {
        ...
        $buffer = [
            ...
            "datestart" => Carbon::create(collect($evt->get('start'))->get('dateTime')),
            "timestart" => Carbon::create(collect($evt->get('start'))->get('dateTime')),
            "dateEnd" => Carbon::create(collect($evt->get('end'))->get('dateTime')),
            "timeEnd" => Carbon::create(collect($evt->get('end'))->get('dateTime')),
        ...
        if ($evt->has('description')) {
            //$events[$i] += ["description" => $event['description']];
            $buffer["description"] = $evt->get('description');
        }
        if ($evt->has('location')) {
            //$events[$i] += ["location" => $event['location']];
            $buffer["description"] = $evt->get('location');
        }
        $events[$i] = $buffer;
...
if (sizeof($events) != 0) {
    if (option('benediktengel.google-calendar-plugin.upcoming') == true && isset($events[0]['dateEnd'])) {
        for ($s=0; $s < sizeof($events); $s++) {
            ...
            if ($today->lte($events[$s]['dateEnd']) && isset($events[$s]['title'])) {
